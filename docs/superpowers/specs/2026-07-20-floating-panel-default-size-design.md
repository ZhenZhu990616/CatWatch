# CatWatch 首次悬浮窗默认尺寸设计

## 目标

将 CatWatch 首次启动时的悬浮窗默认尺寸设为 520×320 点，宽高比约为 16:10，使文本回答有足够的横向阅读空间，同时避免窗口过扁或占用过多屏幕。
...
源代码提交到新的私有 GitHub 仓库 `ZhenZhu990616/CatWatch`。签名后的应用打包为版本化 DMG，并作为 GitHub Release 资产上传；本地 `.build` 与临时发布产物不进入 Git 历史。
